...
function errorText(error) { return error.response?.data?.message || 'Content Studio could not reach your PC. Start the local studio, then open http://127.0.0.1:5055/content-studio on this computer.'; }
        ...
        <div className="cs-workspace">
          ...
          {version && <section className="cs-panel cs-detail-panel"><div className="cs-tabs" role="tablist" aria-label="Campaign details">{[{ id: 'direction', label: 'Direction' }, { id: 'script', label: 'Script' }, { id: 'captions', label: 'Captions' }, { id: 'exports', label: 'Downloads' }].map(tab => <button role="tab" id={`tab-${tab.id}`} aria-controls={`panel-${tab.id}`} aria-selected={view === tab.id} key={tab.id} onClick={() => setView(tab.id)}>{tab.label}{tab.id === 'exports' && version.outputs.length > 0 && <span>{version.outputs.length}</span>}</button>)}</div>
            <div role="tabpanel" id={`panel-${view}`} aria-labelledby={`tab-${view}`} className="cs-tab-content">
              ...
              {view === 'exports' && <>{version.outputs.length ? <div className="cs-exports">{version.outputs.map(output => <div className="cs-export" key={output.id}><span className="cs-export-icon">{output.format === 'video' ? <Film size={19} /> : <ImageIcon size={19} />}</span><div><strong>{output.cover ? 'Video cover' : output.format === 'carousel' ? `Carousel · Slide ${output.slide + 1}` : formats.find(value => value.id === output.format)?.label}</strong><small>{output.width} × {output.height} · {output.format === 'video' ? `MP4 · ${Math.round(output.duration)}s` : 'PNG'}</small></div><button className="cs-icon-button" disabled={Boolean(acting)} onClick={() => download(output)} aria-label={`Download ${output.cover ? 'cover' : output.id}`}><Download size={18} /></button></div>)}</div> : <div className="cs-no-exports"><Download size={24} /><p>{busy ? 'Finished files will appear here as they render.' : 'This version has no finished exports yet.'}</p>{active && !busy && !versionPlan.requiredAssets.length && <button className="cs-button cs-secondary" disabled={Boolean(acting)} onClick={() => perform('Queuing export', async () => { const { data } = await api.post(`${BASE}/projects/${project.id}/jobs`, { kind: 'render' }); applyProject(data.project); })}>Render exports</button>}</div>}<button className="cs-text-button" onClick={() => downloadText('veylo-campaign-copy.txt', Object.entries(versionPlan.captions).map(([platform, text]) => `${platform.toUpperCase()}\n\n${text}\n\n${versionPlan.hashtags.join(' ')}`).join('\n\n----------------\n\n'))}><Download size={15} />Download all posting copy</button></>}
            </div>
