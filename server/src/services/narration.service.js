...
export async function generateNarration(delivery) {
  ...
  const voice = process.env.DEEPGRAM_TTS_MODEL || 'aura-2-athena-en';
  const response = await fetch(`https://api.deepgram.com/v1/speak?model=${encodeURIComponent(voice)}&encoding=mp3`, { method: 'POST', headers: { Authorization: `Token ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ text }), signal: AbortSignal.timeout(90_000) });
  if (!response.ok) {
