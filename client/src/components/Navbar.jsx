...
import { Menu, X, ArrowUpRight, ArrowRight, LogOut, Settings } from 'lucide-react';
...
export default function Navbar({ user, onLogout }) {
 ...
 {user ? <><Link className="v-nav-signin" to="/settings" aria-label="Account settings"><Settings size={17} /></Link><button className="v-nav-signin" onClick={onLogout} aria-label="Sign out"><LogOut size={17} /></button></> : <Link className="v-nav-signin" to="/signin">Sign in</Link>}
 <Link to={user ? '/dashboard' : '/signup'} className="v-button v-button-secondary"><span>{user ? 'My deliveries' : 'Get started'}</span><ArrowUpRight size={16} /></Link>
 ...
 <nav aria-label="Mobile navigation">{[...links, ...(user ? [['Account settings', '/settings']] : []), ['Contact', '/contact']].map(([label, path]) => <Link key={path} to={path} onClick={() => setOpen(false)} aria-current={pathname === path ? 'page' : undefined}>{label}<ArrowUpRight size={17} /></Link>)}</nav>
