...
import { Menu, X, ArrowUpRight, ArrowRight, LogOut } from 'lucide-react';
...
export default function Navbar({ user, onLogout }) {
 ...
 {user ? <button className="v-nav-signin" onClick={onLogout} aria-label="Sign out"><LogOut size={17} /></button> : <Link className="v-nav-signin" to="/signin">Sign in</Link>}
 ...
 <nav aria-label="Mobile navigation">{[...links, ['Contact', '/contact']].map(([label, path]) => <Link key={path} to={path} onClick={() => setOpen(false)} aria-current={pathname === path ? 'page' : undefined}>{label}<ArrowUpRight size={17} /></Link>)}</nav>
