import { CURATED_DELIVERY_SOUNDTRACKS } from './deliveryFormats.js';

// The legacy Photo Story creator now uses the same server-approved catalogue as
// Delivery V2. `audioUrl` and `bestFor` preserve its existing form contract.
export const CURATED_SOUNDTRACKS = CURATED_DELIVERY_SOUNDTRACKS.map(track => ({
  ...track,
  audioUrl: track.url,
  bestFor: track.tags
}));
...
export const SOUNDTRACK_GENRES = [
  'All',
  ...new Set(CURATED_SOUNDTRACKS.map(track => track.genre))
];
