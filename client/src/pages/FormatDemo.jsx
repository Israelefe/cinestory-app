    ...
    {!started ? <main className="fd-reveal-opening">
      ...
      {!finished ? <div className="fd-reveal-room">
        <div className="fd-reveal-frame-shell" {...formatFrameAttributes(activeFrame)} style={formatFrameStyle(activeFrame)}>
          ...
          <AnimatePresence mode="wait">{uncovered && <motion.figure className={'fd-reveal-portrait is-effect-' + ((index % 4) + 1)} {...formatFrameAttributes(activeFrame)} style={formatFrameStyle(activeFrame)} key={activePhoto.url || activePhoto.name} initial={reduced ? false : { opacity: .35, ...motionEffect.initial }} animate={{ opacity: 1, ...motionEffect.animate }} exit={{ opacity: 0, scale: .985 }} transition={{ duration: reduced ? 0 : 1.15, ease: [0.22, 1, 0.36, 1] }}><motion.div animate={frameMotionValues(activeFrame, reduced)} transition={frameMotionTransition(activeFrame, index, reduced)}><Photo name={activePhoto.name} url={activePhoto.url} alt={activePhoto.alt} style={activeFrame.focalPoint ? { objectPosition: activeFrame.focalPoint } : undefined} eager sizes="(max-width: 767px) 100vw, 72vw" /></motion.div><motion.i className="fd-reveal-light" initial={reduced ? false : { x: '-130%', opacity: 0 }} animate={{ x: '180%', opacity: [0, .8, 0] }} transition={{ duration: reduced ? 0 : 1.1, delay: reduced ? 0 : .2, ease: 'easeInOut' }} /></motion.figure>}</AnimatePresence>
