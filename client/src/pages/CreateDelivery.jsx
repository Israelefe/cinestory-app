...
export default function CreateDelivery({ user }) {
  ...
  async function saveReview() {
    ...
    const response = await api.patch(`/v1/deliveries/${delivery._id}/review`, { title: delivery.creativeDirection.title, openingLine: delivery.creativeDirection.openingLine, closingLine: delivery.creativeDirection.closingLine, palette: delivery.creativeDirection.palette, typography: delivery.creativeDirection.typography, pace: delivery.creativeDirection.pace, variation: delivery.creativeDirection.variation, sections: (delivery.creativeDirection.sections || []).map(({ id, title, subtitle, label, delivery: deliveryLabel, layout }) => ({ id, title, subtitle: subtitle || '', label: label || '', delivery: deliveryLabel || '', layout })), frames: delivery.creativeDirection.frames.map(({ assetId, headline, caption, eventType, campaignType }) => ({ assetId, headline: headline || '', caption: caption.trim(), eventType: eventType || '', campaignType: campaignType || '' })), assetOrder: ordered.map(asset => asset.assetId) });
