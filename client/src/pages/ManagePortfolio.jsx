...
export default function ManagePortfolio() {
  ...
  useEffect(() => {
    Promise.allSettled([api.get('/v1/portfolios/mine'), api.get('/v1/portfolios/sources')]).then(([profile, sourceList]) => {
      if (profile.status === 'fulfilled') { setForm({ ...empty, ...profile.value.data.data }); setStatus(profile.value.data.status); setAccess(profile.value.data.access); }
  ...
  async function directPortfolio() {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 300; attempt += 1) {
        ...
        if (data.data.status === 'review') { const fresh = await api.get('/v1/portfolios/mine'); setForm({ ...empty, ...fresh.data.data }); toast.success('Your portfolio direction is ready to review.'); return; }
